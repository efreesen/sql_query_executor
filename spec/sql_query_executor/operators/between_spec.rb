...
describe SqlQueryExecutor::Operators::Between do
  describe "#execute!" do
    ...
    context 'finds element' do
      subject { described_class.new("id between 2 and 4", data) }

      it 'returns filtered collection' do
        expect(subject.execute!).to eq([OpenStruct.new({id: 2}), OpenStruct.new({id: 3}), OpenStruct.new({id: 4})])
      end
    end
    ...
    context 'does not find element' do
      context 'range out of reach' do
        it 'returns filtered collection' do
          operator = described_class.new("id between 8 and 10", data)
      ...
      context 'range overlaps' do
        it 'returns empty array' do
          operator = described_class.new("id between 5 and 3", data)
          expect(operator.execute!).to eq([])
        end
      end
    end
